<template>
  ...
  <div class="px-5" style="text-align: center;">
    <button class="btn btn-primary btn-lg btn-block" type="button" v-on:click="creatEvent()">Create!</button>
  </div>
</template>
  ...
  methods: {
    ...
    creatEvent(){
      ...
      if(validCheck !== null) {
        ...
        this.axios.post('http://localhost:4941/api/v1/events', params, headers)
            .then((res)=>{
              console.log(res.data)
              this.putEventImage(res.data.eventId)
              this.$router.push('/')
            }, (error) => {
              this.error = error;
              this.errorFlag = true;
              alert("Creat failed! Please create again")
              location.reload()
            })
      }
    },
    validFunction(){
      ...
      console.log(this.capacityCheck)
