<template>
  <nav class="navbar navbar-expand-lg navbar-light" style="background: peachpuff; height: 50px">
    ...
    <a class="navbar-brand" href="/">SignUpPage</a>
    ...
    <div class="container">
      ...
      <form name="login">
        ...
        <div style="margin-left: 40%;margin-top:15px;width: 20%;">
          <button type="button" class="btn btn-primary btn-lg btn-block" v-on:click="userSignUp()">Sign Up</button>
        </div>
      </form>
    </div>
  </div>
  ...
  methods:{
    ...
    userSignUp() {
      if (this.email === "" || this.password === "" || this.firstName === "" || this.lastName === "") {
        alert("Please entry complete information!")
      }else if(this.password.length < 8) {
        alert("The password must be at least 8 characters in length.")
      } else if(!this.email.includes("@")) {
        alert("The email address must be syntactically valid.")
      } else{
    ...
    putImage() {
      if (this.selectedFile !== null){
        ...
        const headers = {
          headers: {'X-Authorization': token, "Content-Type": this.selectedFile.type}
        };
          this.axios.put('http://localhost:4941/api/v1/users/' + id + '/image', this.selectedFile, headers)
              .then((res) => {
                console.log(res)
              }, (error) => {
                this.error = error;
